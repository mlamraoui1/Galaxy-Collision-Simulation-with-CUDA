...
void drawUniverse(){
	...
	for (j=0; j<SIZE_UNIVERSE; j++){
		...
		glVertex3f(universe[j].position.x, universe[j].position.y, universe[j].position.z);		
	}	
	/*for (j=0; j<SIZE_DISK_MILKYWAY; j++){
		glColor3f(0.0f, 0.2f, 0.7f);	
		glVertex3f(universe[j].position.x, universe[j].position.y, universe[j].position.z);		
	}
	for (; j<SIZE_DISK_ANDROMEDA; j++){
		glColor3f(7.0f, 0.2f, 0.0f);	
		glVertex3f(universe[j].position.x, universe[j].position.y, universe[j].position.z);		
	}
	for (; j<SIZE_BULGE_MILKYWAY; j++){
		glColor3f(0.0f, 0.0f, 1.0f);	
		glVertex3f(universe[j].position.x, universe[j].position.y, universe[j].position.z);		
	}
	for (; j<SIZE_BULGE_ANDROMEDA; j++){
		glColor3f(1.0f, 0.0f, 0.0f);	
		glVertex3f(universe[j].position.x, universe[j].position.y, universe[j].position.z);		
	}
	for (; j<SIZE_HALO_MILKYWAY; j++){
		glColor3f(0.0f, 0.0f, 0.2f);	
		glVertex3f(universe[j].position.x, universe[j].position.y, universe[j].position.z);		
	}
	for (; j<SIZE_HALO_ANDROMEDA; j++){
...
int main( int argc, char ** argv ) {
	...
	LoadFile(FILENAME, SIZE_UNIVERSE);
	SDL_Event event;
	SDL_Window * window;
